...
{
    if(self.mainAudio.mixStatus == status) return;
    self.mainAudio.mixStatus = status;
    ...
    FSLRunSynchronouslyOnMainQueue(^{
        ...
        if (status == FSLAVMixStatusCompleted) {
            if ([self.mixDelegate respondsToSelector:@selector(didMixedAudioResult:onAudioMix:)]) {
                [self.mixDelegate didMixedAudioResult:self.mainAudio onAudioMix:self];
            }
            if ([self.mixDelegate respondsToSelector:@selector(didCompletedMixAudioOutputPath:onAudioMix:)]) {
                [self.mixDelegate didCompletedMixAudioOutputPath:self.mainAudio.outputFilePath onAudioMix:self];
            }
        }
    });
}
