...
// reference : https://github.com/rs/SDAVAssetExportSession

#import "FSLAVAssetExportSession.h"

@interface FSLAVAssetExportSession ()
{
...
@implementation FSLAVAssetExportSession
{
    NSError *_error;
...
{
    return [FSLAVAssetExportSession.alloc initWithAsset:asset];
}
...
- (void)exportAsynchronouslyWithCompletionHandler:(void (^)(void))handler
{
    ...
    inputQueue =
    inputQueue = dispatch_queue_create("VideoEncoderInputQueue", DISPATCH_QUEUE_SERIAL);
    if (videoTracks.count > 0) {
        [self.videoInput requestMediaDataWhenReadyOnQueue:inputQueue usingBlock:^
         {
    ...
    if (!self.audioOutput) {
        ...
        [self.audioInput requestMediaDataWhenReadyOnQueue:inputQueue usingBlock:^
         {
        ...
        dispatch_async(inputQueue, ^
                       {
                           ...
                           [self reset];
                           [self notifyStatus:AVAssetExportSessionStatusCancelled];
                       });
    }
}

- (void)notifyStatus:(AVAssetExportSessionStatus)status;
{
    if ([self.delegate respondsToSelector:@selector(exportSession:notifyStatus:)]) {
        [self.delegate exportSession:self notifyStatus:status];
    }
}
