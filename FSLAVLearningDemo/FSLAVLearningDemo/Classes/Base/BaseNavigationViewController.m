...
- (void)setNavTitle:(NSString *)navTitle{
    _navTitle = navTitle;
    self.topNavigationBar.titleLab.text = navTitle;
}

- (void)setIsShowBackBtn:(BOOL)isShowBackBtn{
    _isShowBackBtn = isShowBackBtn;
    self.topNavigationBar.backButton.hidden = !_isShowBackBtn;
}

- (void)setIsShowRightBtn:(BOOL)isShowRightBtn{
    _isShowRightBtn = isShowRightBtn;
    self.topNavigationBar.rightButton.hidden = !_isShowRightBtn;
}
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = [[UIApplication sharedApplication] statusBarFrame].size.height > 20 ? 20 : 0;
    if (@available(iOS 11.0, *)) {
        _topContentOffset = statusBarHeight + kDefaultTopNavigationBarHeight;
    }
