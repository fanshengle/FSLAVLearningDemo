...
@interface ZQPlayerMaskView ()<FSLAVPlayerDelegate,UIGestureRecognizerDelegate> {
    BOOL _isDragSlider;
}
...
- (void)playWithVideoUrl:(NSString *)videoUrl {
    [_player stop];
    _player.currentURLStr = videoUrl;
...
- (void)didChangedPlayState:(FSLAVPlayerState)state player:(FSLAVPlayer *)player {
    ...
    else if (state == FSLAVPlayerStatePause) {
    ...
    else if (state == FSLAVPlayerStateReadyToPlay) {
        _playBtn.selected = YES;
        self.backgroundImage.image = nil;
        if (_player.isBuffering) {
            [self startLoading];
...
- (FSLAVPlayer *)player {
    if (!_player) {
        ...
        _player = [[FSLAVPlayer alloc] initWithURL:videoUrl];
